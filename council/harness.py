...
from council.snapshot import build_snapshot
from council.council import council_decision, SEATS
from council.scorer import realized_outcome, ape, gonogo_correct

logger = logging.getLogger(__name__)
OUT_DIR = D.ROOT / "council" / "outputs"
...
def _build_report(res: pd.DataFrame, cand: pd.DataFrame, params: Dict[str, Any]) -> str:
    ...
    L: List[str] = [
        ...
        f"`earnest-proforma-final-1.6/data/main-data-v2-stitched.csv`.",
        "",
...
def _signal_report(ev: Dict[str, Any], m: pd.DataFrame) -> str:
    ...
    L = [
        ...
        f"focal builds (openings 2021–2024, single source `earnest-proforma-final-1.6/...stitched.csv`, data {dr}).",
        "",
