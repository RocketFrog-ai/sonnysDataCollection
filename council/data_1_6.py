...
`earnest-proforma-final-1.6/data/main-data-v2-stitched.csv`.
...
points at the 1.6 CSV and adds the per-site maturity fields the backtest needs
(`mat_n`, `mature_level`). Copies the 6-line `haversine_km` locally so this package never
imports `modelling/data.py` (which hardcodes the 2.0 CSV and side-loads coldstart_model).
...
MAIN_CSV = ROOT / "earnest-proforma-final-1.6" / "data" / "main-data-v2-stitched.csv"
EARTH_KM = 6371.0088
...
def load_panel_1_6() -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Monthly panel `df` + per-site table `site`, from the 1.6 stitched CSV. Cached for the process.
