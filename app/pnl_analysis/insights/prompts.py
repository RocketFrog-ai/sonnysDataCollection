...
SYSTEM_PROMPT = (
   ...
   "\n"
    ...
    "Rules:\n"
    "- SIMPLE, DATA-DRIVEN ENGLISH: write in clear, plain language a non-finance reader follows on the first read. Lead every point with a NUMBER or a dated fact, then a short 'because ...' clause on why it matters — not a paragraph. Cut hype adjectives ('strong', 'robust', 'healthy', 'attractive', 'impressive', 'solid'); let the figure carry the judgement. Spell out EVERY abbreviation in full: 'year-over-year' (never 'YoY'), 'percentage points' (never 'pp'), 'month-over-month' (never 'MoM'), 'average revenue per wash' (never 'ASP'), 'per month' (never '/mo'), 'per year' (never '/yr').\n"
    "- NO REPETITION: never restate the same figure or point in two places; each section must add something NEW. Cover more distinct points, but keep every one short.\n"
...
"  {\"headline\": \"\", \"bullets\": [], \"signal\": \"neutral\"}\n\n"    "\"washes\" must have this exact shape:\n"
    ...
    "number-led sentence (roughly 12-20 words) — a marker, not a paragraph. Start each with the BOLDED section label, then "
    "' — ', then the key figure(s) in **bold** and a brief 'so what' in plain words (no hype adjectives). Do NOT reuse a "
    "figure already cited in another bullet — every bullet adds a new fact. Every number is a multi-month window; do NOT "
    "include any data-note / coverage / caveat section:\n\n"
    ...
    "head-to-head fight with an entrenched operator. Then trace how the field FILLED IN across **2023, 2024 and 2025** "
    "from the per-site opening dates in the market-context block (which sites were operating each year, any new "
    "entrant), so the reader sees whether the trade area is getting more crowded or holding steady.\n\n"
