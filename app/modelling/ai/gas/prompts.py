...
def build_observation_prompt(feature_narratives: List[Dict[str, Any]]) -> str:
    ...
    return (
        "Gas station feature summaries:\n"
        + "\n".join(feature_lines)
        + "\n\nWrite one Observation paragraph (2-4 sentences) with the overall gas ecosystem picture."
    )
...
def build_conclusion_prompt(feature_narratives: List[Dict[str, Any]]) -> str:
    ...
    return (
        "Gas station feature summaries:\n"
        + "\n".join(feature_lines)
        + "\n\nWrite a 1-2 sentence Conclusion on overall gas station impact for car wash demand."
    )
