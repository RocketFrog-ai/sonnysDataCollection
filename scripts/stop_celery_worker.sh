...
PID_FILE="$RFW_HOME/sonnysDataCollection/celery_$ENV_NAME.pid"
...
if ps -p $PID > /dev/null 2>&1; then
  echo "🛑 Stopping Celery worker ($ENV_NAME) with PID $PID ..."
  kill $PID
  sleep 2
  if ps -p $PID > /dev/null 2>&1; then
  ...
  rm -f "$PID_FILE"
  echo "✅ Celery worker ($ENV_NAME) stopped successfully."
else
