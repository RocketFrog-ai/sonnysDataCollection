...
PYTHONPATH="${REPO_ROOT}/app/site_analysis/features/competitors:${REPO_ROOT}/app/site_analysis/features${PYTHONPATH:+:$PYTHONPATH}" nohup "${CONDA_PREFIX}/bin/celery" -A app.tasks.celery_app worker --loglevel=info --pool=prefork>"$bs_log_pfn" 2>&1 &
