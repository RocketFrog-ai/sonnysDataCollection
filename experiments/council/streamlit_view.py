...
from experiments.council import config
from experiments.council.snapshot import build_live_snapshot
from experiments.council.council import council_decision, council_notes
...
def render_reports() -> None:
    """Show the honest backtest report + the rebuild meeting notes (from experiments/council/ files) in expanders."""
    ...
    with st.expander("Backtest report — honest, out-of-fold (does it beat 'always build'?)"):
        st.markdown(report.read_text() if report.exists()
                    else "_No backtest yet — run `python -m experiments.council.harness`._")
