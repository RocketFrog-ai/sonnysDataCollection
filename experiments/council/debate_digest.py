...
_SYS = (
    ...
    "Then a final 'takeaway': 1-2 sentences on why the committee landed where it did.\n"
...
def _fallback(payload: List[dict], moves: Dict[int, List[str]]) -> Dict[str, Any]:
    """Deterministic, clean per-round summary (no LLM, no ellipsis) — used if Azure is unreachable."""
...
def summarize_debate(messages: List[dict], belief_history: Dict[str, List[dict]], verdict: str,
    ...
    try:
        ...
        if not clean:
            return _fallback(payload, moves)
        return {"rounds": clean, "takeaway": str(j.get("takeaway") or "").strip()[:500]}
    except Exception:
        return _fallback(payload, moves)
