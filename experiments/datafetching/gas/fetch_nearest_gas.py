"""
Fetch nearest gas station for each address and add columns in-place to Proforma-v2-data.xlsx.
No separate output file: read and write the same Excel, keep adding columns for gas (and other features later).

Input/Output: datafetching/input_data/Proforma-v2-data.xlsx (column: Address)

Usage (from project root):
  python datafetching/fetch_nearest_gas.py [start_index] [end_index]

Run in order (e.g. 0 100, then 100 200, ...) to fill the new columns.
"""
...
from app.site_analysis.features.nearbyGasStations.get_nearby_gas_stations import get_nearest_gas_station_only
