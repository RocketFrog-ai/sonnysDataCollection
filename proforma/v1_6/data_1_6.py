"""
Single-source loader for the council backtest — reads ONLY
...
imports app/pnl_analysis/modelling/data.py (which now imports proforma.v1_5.models.coldstart directly).
...
PROFORMA = Path(__file__).resolve().parents[1]           # proforma/
# The council used to read its own copy of this panel, under the old 1.6 tree. That copy and
# the old 2.0 mirror were verified byte-identical (same sha256, 13,269,792 bytes) before being
# collapsed into this single shared store, so the council reads exactly the bytes it always did.
MAIN_CSV = PROFORMA / "data" / "panel" / "main-data-v2-stitched.csv"
EARTH_KM = 6371.0088
